...
10. Any failure: status `failed`, the report still goes out with the error, and if even the run log could not be written the report says so. Work that did reach the sheet before the failure is recorded in the inbox (new rows are marked as soon as they are written; each revision, retraction and deletion is recorded as it completes), so a rerun neither redoes nor duplicates it; whatever was not applied stays pending. Sheets calls are retried with a fresh request each time (gspread rewrites a batch request's ranges in place, which on 22 Sep 2026 turned a retried quota error into `'Tab'!'Tab'!'Tab'!B209`), a 429 waits 20 s per attempt, and when more than five revisions are due they are paced two seconds apart to stay under Google's per-minute write quota.
...
| `/backfill` … (`/done` / `/cancel`) | private | group members | paste older messages; Telegram cuts long pastes into several messages, so the import starts 20 s after the last part or at once on `/done` (a `/backfill` whose own text is 3,500+ characters is treated as the first part, not imported at once: on 22 Sep 2026 a 7,900-character paste sent with the command lost its second half that way); the bot's own messages are left out; the reply lists what was held as a duplicate; then a sync runs |
...
| `/report [currency]` | group or private | group members | spending by month in the default currency or the one named in any spelling (`€`, `euro`, `usd`, `US Dollar`, `lira`, `tl`); `summary.monthly_totals` reads the transactions tab and the Summary's Exchange rates table once each and reproduces the tab's arithmetic (amount × rate of the row's currency ÷ rate of the target), so the figures equal the tab's; text amounts are ignored, blank currencies count as the default, rows without a rate are counted but reported as left out; no model call anywhere, the currency comes from `extractor.CURRENCY_ALIASES` |
| `@botname …` | group | anyone in the group | a mention of the bot (a `MessageEntity.MENTION` equal to its username, `bot.MentionsBot`) is routed by `on_mention`: alone or before `/start`, `/help` or an unknown text → the `/start` answer; before `/sync`, `/status`, `/review`, `/report`, `/backfill` → that command with the words after it; before a text containing a digit → stored as an expense note. Such messages never reach the inbox as notes (until 23 Sep 2026 "@botname /start" was stored and later skipped by Claude) |
| `/status`, `/start`, `/help` | anywhere | anyone | integration checklist with fixes, and the command list |

### Messy input the prompt handles

- Descriptions are the author's words kept whole: parentheses, remarks, names and emojis stay ("WiFi Internet Bill (havale to Erkan)", "Trendyol 🛒 (incl 🖥️ monitor)"); only the amount, its currency words and a trailing colon or dash are taken out. A description over several lines is joined with a space; words after the amount are appended; a lone emoji becomes its English word followed by the emoji ("Eggplant 🍆").
- "++" before an amount means money that came back (bank profit, a refund): the row is written with a negative amount, so the Summary subtracts it. `extractor.audit` refuses a negative amount when the message has no "++".
- Arithmetic under an item ("2192-1200 / = 992 TL") is one amount (992), kept in the description; `looks_truncated` collapses such expressions before counting amounts, so they no longer trigger a false "cut short" retry (this was the "rejected answer" of 22 Sep 2026). What counts as an amount for that check is what the household writes as one: a number alone on its line (optionally with a currency word, "++" or a remark in parentheses) or a number followed by a currency word; postal codes, coordinates and numbers inside prose do not count (an address message with "34100, 34100" and GPS coordinates used to look like five amounts).
- An amount-only message that follows a complete transaction from the same sender ("Cafe / 225 TL", then "295 TL") is a second purchase with the same description, its own row on the amount message; an amount-only message after a description without an amount is still merged into it.
- A description that is only an emoji is written as the word it depicts ("🍆" → "Eggplant", Groceries).
- An amount with no description inside a message ("...305", "207**") is never a row and never vanishes: the other items are returned and the message is flagged with the unexplained amount in its note.
...
Telegram bots never receive messages sent before they joined. History comes in through `/backfill` (paste) or `backfill FILE`. A re-pasted message with the same sender and send time (±120 s) as exactly one stored message but a different text is queued as a revision of that message (`SheetStore.revise_messages`), so a corrected dump updates the rows the wrong copy produced; when several stored messages fall in that window (Telegram copies quick consecutive messages as one block) the block is skipped if it equals their joined texts and held as `duplicate` otherwise; a revision of a message whose rows carry no provenance note (written before notes existed) is held by the sync with "rows left unchanged" instead of appending a second copy, and `resync` leaves such messages alone; lines starting with "..." are listed in the reply as probably cut by Telegram's copy. `python bot.py resync [--since DATE] [--dry-run]` queues every message that owns rows as a revision and runs one sync, so existing rows follow new prompt rules. Otherwise an import queues nothing that is already there: a message the inbox has (same id, or the same normalised text within 120 s of a stored send time; Telegram's copy rounds 17:04:59 to 17:05:00, which on 22 Sep 2026 let two live messages through a minute-precision key and put 13 duplicate rows on the sheet, removed the same day through their provenance notes) is skipped; the same text stored on the same day at another time is held as `duplicate` for `/review`; a message that repeats a row on the transactions tab (`SheetStore.transaction_index`, compared by `backfill.same_item`: the same describing words once amounts, currency words, digits, symbols, case and the prompt's "Groceries - " prefix are set aside (as a set, or joined without spaces so "Carre four" is "Carrefour"), the same amount read every way its separators can be meant, and the same currency, the default when none is written; a multi-item message is held when any item repeats a row; the day after midnight is checked against both calendar days) is stored as `duplicate` and never queued until a person decides in `/review`. "Migros 450" repeats "Groceries - Migros · 450 TRY"; "Migros - Water 400" does not repeat "Migros · 400 TRY". Nothing is dismissed by date unless `--from` is given. The earlier *live* cutoff rule, which refused any row dated before the sheet's last entry at every sync, was removed on 19 Sep 2026: it rejected a legitimate multi-day dump posted late (message 52, "Sep 3 … Sep 15"), because the sheet's last date came from the bot's own previous run, not from complete hand entry.
...
`python bot.py init-sheet` builds a report tab (`SUMMARY_TAB`, default `Summary`) over the transactions tab, all formulas, styled with green header bands, tinted column headers, alternating row shading, a donut chart and a column chart: spend, share and count per category, spend per month (months found with `SORT(UNIQUE(EOMONTH(…)))`), totals per currency, the ten largest expenses, a pie chart by category and a column chart by month. Cell B3 is a dropdown (`ONE_OF_LIST` over the supported currencies, prefilled from `DEFAULT_CURRENCY`). Every amount is converted into it: an exchange-rate table (`A80:B84`, one editable cell per currency, the value of one unit in any common reference currency; `init-sheet` prefills 1 for the default, `GOOGLEFINANCE("CURRENCY:XXXDEF")` for the others and leaves TOMAN blank) feeds `ARRAYFORMULA(SUMPRODUCT(condition × IF(ISNUMBER(amount), amount, 0) × IFERROR(1×VLOOKUP(currency-or-default, rates, 2), 0) / VLOOKUP($B$3, rates, 2)))` for every total, `SUMPRODUCT(condition × ISNUMBER(amount))` for every count, and a `FILTER`/`SORTN` over the same converted expression for the ten largest expenses (`SUMIFS` cannot be used across tabs of different height). `SUMPRODUCT` is used so that notes typed across a row (merged cells with text in the amount column) count as 0 instead of breaking the tab. `--rewrite` reads the old tab's rate cells (`summary.read_rates`) and writes them back, so personal rates survive a rebuild. The category names in `A7:A25` of that tab are what the category dropdown on the transactions tab offers, because `init-sheet` points the dropdown's data validation there, so a category is added by typing it in the next free cell. An existing tab of that name is kept unless `--rewrite` is given, in which case it is deleted and rebuilt (the transactions tab is never touched).
...
- A message is inserted at most once (status column plus de-duplication by message id; imports also match against the sheet's own rows by day, wording, amount and currency).
- `python bot.py categorise [--rows FIRST:LAST] [--dry-run]` fills the empty category cells of rows that have a description (one plain-JSON call at the configured effort, categories limited to the sheet's list, a retry for whatever came back unusable), re-checks that each cell is still empty right before writing, and touches nothing else; rows without a description are skipped.
